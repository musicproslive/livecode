...
class sdbclass 
	{
		...
		public function getdbcontentshtml_cond($table,$cond="1",$echo=false)
			{
				$fn_sql		=	"select * from $table where $cond";
				$fn_res		=	$this->db_query($fn_sql,$echo);
				$arrcnt		=	-1;
		...
		//fetching with query
		public function getdbcontentshtml_sql($sql,$echo=false)
			{	
				$fn_res		=	$this->db_query($sql,$echo);
				$arrcnt		=	-1;
				$dataarr	=	array();
		...
		public function getdbsinglevalue_cond($table,$field,$cond="1",$echo=false)
			{
				...
				if($data)	return htmlentities(stripslashes($data[0]["fieldname"]), ENT_QUOTES);
				else		return false;
			}
