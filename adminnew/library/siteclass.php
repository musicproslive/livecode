...
//				echo $des;exit;
				...
				if(strtolower($ftype)=='jpg')	imagejpeg($thumb, $dest_file,100); 
				if(strtolower($ftype)=='gif')	imagegif($thumb, $dest_file); 
				if(strtolower($ftype)=='png')	imagepng($thumb, $dest_file,9); 
		...
		function sendSmtpMail($to, $subject, $message,$from = "livemusictutor.com"){//to send mail
			$headers	= "MIME-Version: 1.0\r\n";
			$headers .= "Content-Type: text/html; charset=iso-8859-1\n"; //charset=iso-8859-1\r\n";
			$headers .= "From: Live Music Tutor <".$from.">\n";
			...
			if(mail($to,$subject,$message,$headers,$bounceString)) 
				return true; 
			else 
				return false;
			//teacher1@livemusictutor.com	
		}
		...
		function getCsvFile($sql,$headArr,$fieldArr,$filename="ResultRecords")
			{
				...
				$finalArray		=	array();
				while($dataObject = mysqli_fetch_array($this->con,$result)) 
					{	
		...
		function getRealEscapeData($arr,$exceptions=array())
			{
				if(!function_exists("callBackFngetRealEscapeData"))
					{
						function callBackFngetRealEscapeData(&$item, $key,$exceptions)
							{   $con = new sdbclass();
								...
								$item	=	mysqli_real_escape_string($con->con,$item);
							}
					}
				if(is_array($arr))
					{
				...
				else	return mysqli_real_escape_string($this->con,$arr);
			}
...
class spaging2
	{
		...
		public	$prevCaption			=	"Prev";
		public	$containerTag			=	"li";
		public	$containerLinkParams	=	"class=''";
		public	$containerStatParams	=	"class='opened'";
		public	$outerTag				=	"ul";
		...
		function geturl($param,$sql)
			{
				...
				$res							=	mysqli_query($this->con,$sql_q);
				return $res;
			}
				...
				if(!$this->error)
					{
						...
						if($name)
							...
							{
								if($filetype	==	"1") $des	.=	"/".$this->filename;
								if($filetype	==	"2")
									{
								...
								if($filetype	==	"2")
									{
										...
										$nName		=	str_replace(".tmp","",$tmpfname.".".$ext);
										rename($tmpfname,$nName);
