...
*************** ***********************************************************************/
class deletedContent extends modelclass
	{
		public function deletedContentListing()
			{					
				$alb	= 	new albumlist();
				$allData = $alb -> getDelImages();
				return 	$allData;				
			}		 
		
		public function deletedContentApporve(){
				if(!empty($_POST)){
				$alb	= 	new albumlist();
				...
				//$data['approved_by'] = $_SESSION['USER_LOGIN']['USER_NAME'];			
				$approveBox = 0;	
				$deletestatus = 0;
				$caption = "Disapproved";
				...
				$data['image_owner_id']=$result[0]['image_owner_id'];
			...
			$this->redirectAction($err,"Listing","deletedContent.php");	
		}
		
		public function deletedContentDelete(){
				//print_r ($_POST['imgId']);
				...
				$data['image_owner_id']=$result2[0]['image_owner_id'];
