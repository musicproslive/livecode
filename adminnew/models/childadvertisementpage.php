...
******************************************************************************************/
class childadvertisementPage extends modelclass
	{
		...
		public function childadvertisementPageFetchAdvertisements()
			{
				...
				$sql 				= 	"SELECT count(*) FROM `tblchildadvertisement_pages` WHERE `is_deleted`=0 ".$searchSql;
				...
				$data['total']		= 	$total;
				$data['rows'] 		= 	array();
				$sql 				= 	"SELECT * FROM `tblchildadvertisement_pages` WHERE `is_deleted`=0 " .$searchSql." ".$sortSql." ".$limitSql;
				...
				while ($row = mysql_fetch_assoc($results)) 
					{
						...
						$row['sideAds']	=	"<a href=\"childadvertisementPageView.php?id=".$row['refference_name']."\" class=\"Second_link\" >
											<img src=\"../images/view.png\" border=\"0\" title=\"Click here to add details\" ></a>";
						$row['topAds']	=	"<a href=\"childadvertisementPageView.php?id=".$row['refference_name']."_TOP\" class=\"Second_link\" >
											<img src=\"../images/view.png\" border=\"0\" title=\"Click here to add details\" ></a>";
						$data['rows'][] =	 array
							(
		...
		public function childadvertisementPageAddformbtn()
			{
				$this->executeAction(false,"Addform",true);	
			}	
		public function childadvertisementPagesave()
			{
				$data					=	$_POST;
				$ads					=	array();
				$ads['page_name']		=	$data['page_name'];
				$ads['refference_name']	=	$data['refference_name'];
				$dataIns1				=	$this->populateDbArray("tblchildadvertisement_pages",$ads);
				$adsID					=	$this->db_insert("tblchildadvertisement_pages",$dataIns1);
			}
		
		public function childadvertisementPageDeletedata()
			{
		...
		public function deleteadvertisement($id)
			{
				$query 		= 	"UPDATE tblchildadvertisement_pages  SET is_deleted='1' WHERE advertisement_id='$id'";	
