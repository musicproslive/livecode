...
******************************************************************************************/
class advertisementPageView extends modelclass
	{
		public function advertisementPageViewListing()
			{
				//Distroying Page Number and Search String Session value for all other page.
				$cls			=	new adminUser();
				$cls->unsetSelectionRetention(array($this->getPageName(),'advertisementPage.php'));
				if(!isset($_GET['flag']))
				$flag	=	"OFF";
				else
				$flag	=	$_GET['flag'];
		...
		public function advertisementPageViewFetch()
			{
				...
				$sql 				= 	"SELECT count(*) FROM `tbladvertisement_listing` WHERE `is_deleted`=0 ".$searchSql;
				...
				$data['total']		= 	$total;
				$data['rows'] 		= 	array();
				$sql 				= 	"SELECT * FROM `tbladvertisement_listing` WHERE `is_deleted`=0 " .$searchSql."".$sortSql;
				...
				while ($row = mysql_fetch_assoc($results)) 
					{
						$i++;			
						$row['delete']	=	"<a href=\"advertisementPage.php?actionvar=Deletedata&id=".$row['advertisement_id']."\" class=\"Second_link\" onclick=\"return askDelete()\"\">
											<img src=\"../images/delete.png\" border=\"0\" title=\"Click here to delete\" \"></a>";
						
						$row['view']	=	"<a href=\"advertisementPageView.php?id=".$row['refference_name']."\" class=\"Second_link\" >
											<img src=\"../images/view.png\" border=\"0\" title=\"Click here to delete\" ></a>";
						$row['mode']	=	"Nornal Add";
						if($row["advt_mode"]==1){
							$row['mode']	= "Google Add";
						}
						$row['stat']		=	"<a href=\"advertisementPageView.php?type=0&actionvar=Stauschange&advid=".$row['advertisementlisting_id']."\" class=\"Second_link\">
							<img src=\"../images/active.gif\" border=\"0\" title=\"Click here to disapprove\"></a>";
							
							
						if($row["status"]==0){
							$row['stat']	= 	"<a href=\"advertisementPageView.php?type=1&actionvar=Stauschange&advid=".$row['advertisementlisting_id']."\" class=\"Second_link\">
							<img src=\"../images/inactive.gif\" border=\"0\" title=\"Click here to approve\"></a>";
						}
						
							$row['delete']	= 	"<a href=\"advertisementPageView.php?actionvar=Delete&advid=".$row['advertisementlisting_id']."\" class=\"Second_link\" onclick=\"return askDelete()\"\">
											<img src=\"../images/delete.png\" border=\"0\" title=\"Click here to delete\" \"></a>";					
						if($_GET['flag'] == 'ON')
							$view	=	$this->getAdvertisement($row['advertisementlisting_id']);
						else
							$view	=	'<a href="advertisementPageView.php?actionvar=Viewform&id='.$row['advertisementlisting_id'].'"><img src="../images/edit.gif" border="0" title="Edit add details"></a>';	
		...
		public function advertisementPageViewStauschange(){
				
				$this->db_update("tbladvertisement_listing",array("status"=>$_GET['type']),"advertisementlisting_id=".$_GET['advid'],1);				
				header("Location:advertisementPageView.php?id=".$_SESSION['mode']);	
				exit;	
			}
		
		public function advertisementPageViewViewform(){
				$query		=	"SELECT * FROM `tbladvertisement_listing` WHERE `advertisementlisting_id`=".$_GET["id"];
				$rec		=	end($this->getdbcontents_sql($query));
				return $rec;
				exit;
			}
		public function advertisementPageViewSave()
			{
				$array	=	array();	
				...
				if($this->db_update("tbladvertisement_listing",$array,$this->dbSearchCond('=','advertisementlisting_id',$_POST['id']),0))
					{
		...
		public function getAdvertisement($id)
			{
				$query		=	"SELECT * FROM `tbladvertisement_listing` WHERE `advertisementlisting_id`= $id";
		...
		public function advertisementPageViewDelete(){
			$cls			=	 new advertisementManagement();
			$rec			=	$cls->listAdvById($_GET['advid']);
			print_r($rec);
			if($rec[0]["advt_mode"]==2){
					$cls->unsetImage($rec[0]["image_path"]);
			}			
			$this->dbDelete_cond("tbladvertisement_listing","advertisementlisting_id=".$_GET['advid'],1);				
			header("Location:advertisementPageView.php?id=".$_SESSION['mode']);	
			exit;	
		}
