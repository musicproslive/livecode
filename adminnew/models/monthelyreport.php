...
******************************************************************************************/
class monthelyreport extends modelclass
	{
		public function monthelyreportListing()
			{
				//Distroying Page Number and Search String Session value for all other page.
				$cls			=	new adminUser();
				$cls->unsetSelectionRetention(array($this->getPageName(),"yearlyreport.php"));
				return;
				
			}
		public function monthelyreportFetch(){
					...
					$sql		=	"SELECT count(e.`enrolled_id`) as total,SUM(p.`trans_amount`) as amount, MONTHNAME(e.`created_on`) as month,
				 	...
				 	$sql					.=	" GROUP BY MONTH(e.`created_on`) $searchSql ";
					$resultArry				=	count($this->getdbcontents_sql($sql, 0));
					$total					= 	$resultArry;
					...
					while ($row = mysql_fetch_assoc($results)) 
						{
							$details		=	"<a href='dailyreport.php?year={$row['year']}&month={$row['m']}'><img src='images/inner_details.png' alt='View details' ></a>";
							$i++;
							$data['rows'][] = array(
							'id' => $row['course_id'],
