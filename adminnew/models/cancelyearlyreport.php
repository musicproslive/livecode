...
******************************************************************************************/
class cancelyearlyreport extends modelclass
	{
		public function cancelyearlyreportListing()
			{
		...
		public function cancelyearlyreportFetch(){
					...
					$sql			 = 	"SELECT  count(distinct YEAR(e.`created_on`)) FROM `tblcourse_cancel_transaction` p
					...
					$sql		=	"SELECT count(e.`enrolled_id`) as total,SUM(p.`refunded_amount`) as amount,YEAR(e.`created_on`) as year
									FROM `tblcourse_cancel_transaction` p
									LEFT JOIN `tblcourse_enrollments` e ON p.`enrolled_id`=e.`enrolled_id`
									LEFT JOIN `tblcourses` c  ON e.`course_id`=c.`course_id`							
									WHERE  p.is_refunded=1 AND  e.`enrolled_status_id`=".LMT_CS_ENR_CANCELLED." $searchSql  GROUP BY YEAR(e.`created_on`)  $sortSql $limitSql";
					$results 	= 	$this->db_query($sql,0);
					
					$i			=	$pageStart;
					while ($row = mysql_fetch_assoc($results)) 
						{
							$details		=	"<a href='cancelmonthelyreport.php?year={$row['year']}'><img src='images/inner_details.png' alt='View details' ></a>";
							$i++;
							$data['rows'][] = array(
							'id' => $row['course_id'],
