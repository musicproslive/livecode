		...
		function getCourseDetailStdView($courseCode, $serverOffset = '+00:00', $studentOffset = '+00:00', $dtFmt = '%b %d %Y', $tmFmt = '%h:%i %p')
			{
				$sql = "SELECT C.course_id, C.course_code, C.title, C.description,
						...
						U.first_name AS ins_first_name, U.last_name AS ins_last_name, U.user_id AS ins_id
...
//echo $sql;
	...
	function getStudentsCourseHistory($studId, $dtFmt = '%b %d %Y', $tmFmt = '%h:%i %p', $condition = "", $serverOffset = "+00:00",$studentOffset = "+00:00")
		{	//Commented by Bhaskar
			 $sql1	=	"SELECT E.enrolled_id,E.enrolled_status_id, E.paid_flag,E.panic_flag,E.enrollment_code,C.course_code, C.title, C.course_code,C.course_status_id,U.profile_image,concat(U.first_name,' ',U.last_name)as insname,tct.type,
						...
						ctq.processing_status AS cc_pymnt_process_status, C.course_type_id as type_num,UNIX_TIMESTAMP(CONVERT_TZ(CONCAT(C.start_date, ' ', C.start_time), '$serverOffset', '$studentOffset')) as ctstart, UNIX_TIMESTAMP
(NOW()) as curr 
						...
						LEFT JOIN tbllookup_course_type AS tct ON tct.id = C.course_type_id
						WHERE student_id = $studId $condition and course_code!= '' GROUP BY C.course_id";
						/*$order_bked="ORDER BY C.start_date ASC, C.start_time ASC, E.enrolled_status_id ASC, E.created_on DESC"; */
			//updated by bhaskar
			 $sql2	=	"SELECT E.enrolled_id,E.enrolled_status_id, E.paid_flag,E.panic_flag,E.enrollment_code,C.course_code, C.title, C.course_code,C.course_status_id,U.profile_image,concat(U.first_name,' ',U.last_name)as insname,tct.type,
							I.name as instrument_name, I.instrument_image, DATE_FORMAT(CONVERT_TZ(CONCAT(C.start_date, ' ',
							C.start_time), '$serverOffset', '$studentOffset'), '$dtFmt') AS start_date,
							DATE_FORMAT(CONVERT_TZ(CONCAT(C.start_date, ' ', C.start_time), '$serverOffset',
							'$studentOffset'), '$tmFmt') AS start_time, UNIX_TIMESTAMP(CONCAT(C.start_date, ' ',
							C.start_time))+D.time*60 AS course_server_time, P.cost,D.time, V.id AS video_id, V.archive_id as video_link, N.id AS note_id, R.symbol, C.course_type_id as type_num,UNIX_TIMESTAMP(CONVERT_TZ(CONCAT(C.start_date, ' ', C.start_time), '$serverOffset', '$studentOffset')) as ctstart, UNIX_TIMESTAMP
(NOW()) as curr 
							FROM tblcourse_enrollments AS E
							LEFT JOIN tblcourses AS C ON E.course_id = C.course_id
							LEFT JOIN tblusers AS U ON C.instructor_id = U.user_id
			    ...
			    $sql	=	"SELECT E.enrolled_status_id, E.paid_flag, C.title, C.course_code,
							...
							' ', C.start_time), '$serverOffset', '$studentOffset'), '$tmFmt')
							 AS start_time, D.time, V.id AS video_id , N.id AS note_id,
	 						 (UNIX_TIMESTAMP(CONVERT_TZ(CONCAT(C.start_date, ' ', C.start_time), '$serverOffset', '$studentOffset')) - UNIX_TIMESTAMP(NOW())) as starts_in, UNIX_TIMESTAMP(CONVERT_TZ(CONCAT(C.start_date, ' ', C.start_time), '-05:00', '-05:00')) as ctstart, UNIX_TIMESTAMP
(NOW()) as curr, C.course_type_id as type ,UNIX_TIMESTAMP(CONVERT_TZ(CONCAT(C.start_date, ' ', C.start_time), '-05:00', '-05:00')) as ctstart, UNIX_TIMESTAMP
(NOW()) as curr 
							 FROM tblcourse_enrollments AS E
							 LEFT JOIN tblcourses AS C ON E.course_id = C.course_id
							 LEFT JOIN tblusers AS U ON C.instructor_id = U.user_id
		...
		function getTutorCourses($insId,$insOffset = '+00:00', $serverOffset = '+00:00', $dtFmt = '%b %d %Y', $tmFmt = '%h:%i %p', $cond="")
			{
			    ...
			    $sql	=	"SELECT C.title,C.course_code,C.course_status_id ,C.course_id,
							DATE_FORMAT(CONVERT_TZ(CONCAT(C.start_date, ' ', C.start_time), '$serverOffset','$insOffset'),'$dtFmt') AS start_date, 
							DATE_FORMAT(CONVERT_TZ(CONCAT(C.start_date, ' ', C.start_time), '$serverOffset', '$insOffset'), '$tmFmt') AS start_time ,
							C.panic_flag,P.cost,D.time ,V.id AS video_id, V.archive_id as video_link, N.id AS note_id, R.symbol, C.max_students, 
							COUNT(E.enrolled_id) AS tot_enrolled, I.name AS instrument_name, I.instrument_image, ct.type as type, UNIX_TIMESTAMP(CONCAT(C.start_date, ' ',
							C.start_time))+D.time*60 AS course_server_time,C.course_type_id as type_num,
							UNIX_TIMESTAMP(CONVERT_TZ(CONCAT(C.start_date, ' ', C.start_time), '$serverOffset', '$insOffset')) as ctstart, UNIX_TIMESTAMP
(NOW()) as curr,M.sheet_name, (select ((UNIX_TIMESTAMP (NOW())-UNIX_TIMESTAMP(CONVERT_TZ(CONCAT(C.start_date, ' ', C.start_time), '$serverOffset', '$insOffset')))/60) ) as diff,
DATE_ADD(CONCAT
(C.start_date, ' ', C.start_time),INTERVAL D.time MINUTE) as ctend
								FROM tblcourses AS C 
								...
								AND enrolled_status_id != ".LMT_CS_ENR_CANCELLED."
									LEFT JOIN tblcurrency_type AS R
										ON R.currency_id = P.currency_type
										LEFT JOIN tbllookup_course_type AS ct on ct.id=C.course_type_id
                                    LEFT JOIN tblcourse_sheetmusic AS M
									   ON M.course_id=C.course_id											
								WHERE instructor_id = $insId $cond 
								GROUP BY C.course_id";
							/*	$order =" ORDER BY C.start_date ASC, C.start_time ASC, C.created_on ASC, C.course_status_id ASC";*/
				return $sql;
// AND concat(`start_date`,0x20,`start_time`)>=NOW()
			}
             function getTutorCoursesall($insId,$insOffset = '+00:00', $serverOffset = '+00:00', $dtFmt = '%b %d %Y', $tmFmt = '%h:%i %p', $cond="")
			{
				
				$tz1 = new timeZone();
				$courseTime = date('Y-m-d', $tz1->convertTime($serverOffset, $insOffset, date('Y-m-d H:i:s')));
			   
			    $sql	=	"SELECT C.title,concat(U.first_name,'  ',U.last_name)as insname,U.profile_image,C.course_code,C.course_status_id ,C.course_id,
							DATE_FORMAT(CONVERT_TZ(CONCAT(C.start_date, ' ', C.start_time), '$serverOffset','$insOffset'),'$dtFmt') AS start_date, 
							DATE_FORMAT(CONVERT_TZ(CONCAT(C.start_date, ' ', C.start_time), '$serverOffset', '$insOffset'), '$tmFmt') AS start_time ,C.instructor_id,
							C.panic_flag,P.cost,D.time ,V.id AS video_id, V.archive_id as video_link, N.id AS note_id, R.symbol, C.max_students, 
							C.num_enrolled AS tot_enrolled, count(E.enrolled_id) as tot_enrolled_new, I.name AS instrument_name, I.instrument_image, ct.type as type, UNIX_TIMESTAMP(CONCAT(C.start_date, ' ',
							C.start_time))+D.time*60 AS course_server_time,C.course_type_id as type_num,
							UNIX_TIMESTAMP(CONVERT_TZ(CONCAT(C.start_date, ' ', C.start_time), '$serverOffset', '$insOffset')) as ctstart, UNIX_TIMESTAMP
(NOW()) as curr,M.sheet_name, (select ((UNIX_TIMESTAMP (NOW())-UNIX_TIMESTAMP(CONVERT_TZ(CONCAT(C.start_date, ' ', C.start_time), '$serverOffset', '$insOffset')))/60) ) as diff,
DATE_ADD(CONCAT
(C.start_date, ' ', C.start_time),INTERVAL D.time MINUTE) as ctend
								...
								AND N.note_owner_id = C.instructor_id
								AND N.note_status = 0
									LEFT JOIN tblcourse_enrollments AS E 
										ON C.course_id = E.course_id
...
*/	public function getCourseDetail($courseCode)
	...
	*/
	...
	function getAllPricedCourseTypes($userId)
		...
		function getAllCourseTypessche()
		{
				
				$sql				=	"SELECT id,type FROM tbllookup_course_type where id!=3";
				$result	=	$this->getdbcontents_sql($sql);
			return $result;
		}
		function getAllLevelssche($userID)
		{
				$sql_ins_levels_instu="select group_concat(distinct level_id) as levelids from tbl_instructor_ins_levels   where instructor_id=".$userID;
				$result=$this->getdbcontents_sql($sql_ins_levels_instu);
				if($result[0]['levelids']!=""&&$result[0]['levelids']!="NULL")
				{
					$levelids=$result[0]['levelids'];
				}
				else
				{
					$query				=	"SELECT `instructor_level` FROM `tblusers` WHERE `user_id`= $userId";
				...
				$sql				=	"SELECT id,level_name FROM tbllookup_instructor_level where id!=5 and id in(".$levelids.")";
				$result	=	$this->getdbcontents_sql($sql);
			return $result;
		}
