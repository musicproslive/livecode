	...
	$.addFlex = function (t, p) {
		if (t.grid) return false; //return if already exist
	 	p = $.extend({ //apply default properties
			...
			url: true, //URL if using data from AJAX
			...
			//rpOptions: [10, 20, 30, 50,75,100], //allowed per-page values 
			rpOptions: [25, 50,75,100, 125], //allowed per-page values 
			title: false,
		...
		var g = {
			...
			addData: function (data) { //parse data
				...
				if (p.dataType == 'json') {
					data = $.extend({rows: [], page: 0, qtype: '', query: '', total: 0}, data);
				}
				if (p.preProcess) {
...
//-------------------------------
			...
			buildpager: function () { //rebuild pager based on new properties
				...
				$('input[name=q]', this.sDiv).val(p.query);
				$("select[name=qtype] option[value='"+p.qtype+"']").attr("selected","selected");
				$('.pcontrol input', this.pDiv).val(p.page);
				$('.pcontrol span', this.pDiv).html(p.pages);
				var r1 = (p.page - 1) * p.rp + 1;
				var r2 = Number(r1) + Number(p.rp) - 1;
				if (p.total < r2) {
					r2 = p.total;
				}
				if(p.query){
		...
		if (p.buttons) {
			...
			for (var i = 0; i < p.buttons.length; i++) {
				...
				if (!btn.separator) {
					...
					btnDiv.innerHTML = "<div><span style=\"color:#8C1332;font-size:15px;font-weight:bold;width:80px;border:1px solid #CCCCCC;text-align:center;\">" + btn.name + "</span></div>";
		...
		if (p.usepager) {
			...
			if (p.searchitems) {
				...
				$(g.sDiv).append("<div class='sDiv2'>" + p.findtext + 
						" <input type='text' value='" + p.query +"' size='30' name='q' class='qsbox'  id='searchbox'/> "+
						" <select name='qtype' id='qtype'>" + sopt + "</select> # click Enter to Search</div>");
