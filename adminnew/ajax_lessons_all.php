...
$myTimeZone = !empty($myTimeZone) ? $myTimeZone[0]['gmt'] : '-05:00';
...
$columns = array(
					...
					array( 'db' => 'course_code',  'dt' => 'course_code' ),
...
$dtColumns= array(0=>'insname',1=> 'type',2=> 'course_code',3=> 'start_date',4=> 'start_time',5=> 'time',6=> 'cost');
if($enrl_status==1)
	{
		$display_cols=array(0=>'insname',1=> 'type',2=> 'course_code', 3=> 'start_date',4=> 'start_time',5=> 'time',6=> 'cost',7=>'edit');
	}
	if($enrl_status==2)
	{
		$display_cols=array(0=>'insname',1=> 'users',2=> 'type',3=> 'course_code',4=> 'start_date',5=> 'start_time',6=> 'time',7=> 'cost',8=> 'sheet_music');
	}
	else if($enrl_status==3)
	{
		$display_cols=array(0=>'insname',1=> 'users',2=> 'type',3=> 'course_code',4=> 'start_date',5=> 'start_time',6=> 'time',7=> 'video_link',8=> 'note_id');
	}
	else
	  {
		$display_cols=array(0=>'insname',1=> 'type',2=> 'course_code', 3=> 'start_date',4=> 'start_time',5=> 'time',6=> 'cost',7=>'edit');
...
elseif($_REQUEST['order'][0]['column']==7)
{
	$sorting_column=" cost ";
}
...
$limit=" limit ".$start.",".$length;
			...
			if($enrl_status==1)
			{
				...
				$condition = "   concat(`start_date`,0x20,`start_time`)>=NOW() " ;	
				$condition .= " and (course_status_id=1 or course_status_id=2) ";
				if($_REQUEST['start_date']!="")
				{
					$date_split=explode("/",$_REQUEST['start_date']);
					$starting_date=$date_split[2]."-".$date_split[0]."-".$date_split[1];
					if($_REQUEST['start_date_ed']=="")
					{
						$ending_date=$starting_date;
					}
					else
					{
						$date_split_end=explode("/",$_REQUEST['start_date_ed']);
						$ending_date=$date_split_end[2]."-".$date_split_end[0]."-".$date_split_end[1];
					}
					$condition.=" and C.start_date between '".$starting_date."' and '".$ending_date."'";
				}
			...
			elseif($enrl_status==2)
			{
				$server_time_zone=strtotime(date('Y-m-d H:i:s'));
				$time_Ser=date('Y-m-d H:i:s');
				$condition = "   date(concat(`start_date`,0x20,`start_time`))>=date(NOW())  and (course_status_id=1 or course_status_id=2) " ;	
				if($_REQUEST['start_date']!="")
				{
					$date_split=explode("/",$_REQUEST['start_date']);
					$starting_date=$date_split[2]."-".$date_split[0]."-".$date_split[1];
				...
				//$condition = " course_status_id>0";
				$having_cond=" having count(E.enrolled_id)>0 ";
			}
			elseif($enrl_status==3)
			{
				
				$condition = "  course_status_id=4 " ;	
				if($_REQUEST['end_date']!="")
				{
					$date_split=explode("/",$_REQUEST['end_date']);
					$starting_date=$date_split[2]."-".$date_split[0]."-".$date_split[1];
					if($_REQUEST['end_date_ed']=="")
					{
						$ending_date=$starting_date;
					}
					else
					{
						$date_split_end=explode("/",$_REQUEST['end_date_ed']);
						$ending_date=$date_split_end[2]."-".$date_split_end[0]."-".$date_split_end[1];
					}
					$condition.=" and C.end_time between '".$starting_date."' and '".$ending_date."'";
				}
			...
			elseif($enrl_status==5)
			{
				$server_time_zone=strtotime(date('Y-m-d H:i:s'));
				$time_Ser=date('Y-m-d H:i:s');
				$condition = "  course_status_id=1 and  '$time_Ser'>=DATE_ADD(CONCAT(C.start_date, ' ', C.start_time),INTERVAL D.time MINUTE)" ;	
					$having_cond=" having (diff<=30 and tot_enrolled=0) or '$time_Ser'>=ctend ";
				if($_REQUEST['start_date']!="")
				{
					$date_split=explode("/",$_REQUEST['start_date']);
					$starting_date=$date_split[2]."-".$date_split[1]."-".$date_split[0];
					if($_REQUEST['start_date_ed']=="")
					{
						$ending_date=$starting_date;
					}
					...
					{
						$date_split_end=explode("/",$_REQUEST['start_date_ed']);
						$ending_date=$date_split_end[2]."-".$date_split_end[1]."-".$date_split_end[0];
					}
					$condition.=" and C.start_date between '".$starting_date."' and '".$ending_date."'";
				}
			}
		...
		if($_REQUEST['ser_string']!="")
		{
			$condition.= "  and  (U.first_name like '%".$_REQUEST['ser_string']."%' or U.last_name like '%".$_REQUEST['ser_string']."%' or ct.type like '%".$_REQUEST['ser_string']."%' or I.name like '%".$_REQUEST['ser_string']."%' or C.course_code like '%".$_REQUEST['ser_string']."%') ";
		}
		...
		$booked_less_tot = $db->ExecuteQuery($sql3);
		...
		$booked_less=$db->ExecuteQuery($sql3);
	...
	if (count($booked_less)>0)
	{
		...
		else if($display_cols[$i]=="course_code")
		{
		...
		else if($display_cols[$i]=="edit")
		{
			$row[]='<a href="editLesson.php?courseid='.$data['course_id'].'" class="btn btn-warning" type="button" value="Edit">Edit</a>';
		}
			...
			if($data["video_id"]!="")
							{
								...
								$video_link ='<a href="javascript:void(0)" rel="'.$video_url.'" class="viewVideos" title="live class" ><i class="fa fa-music" alt="Video" title="Click here to view video(s)"></i></a>';
							}
							else
							{
  ...
  <div class="col-md-10">'." ".$stuname[0]['stuname']." ".'</div></div>';
			...
			else if($display_cols[$i]=="video_link")
		{
			...
			if($data["video_id"]!="")
							{
								$video_url=base64_encode(serialize($_REQUEST['user_code']))&base64_encode(serialize($data['course_code']))&$data['video_link']."&Recorded";
								$video_link ='<a href="javascript:void(0)" rel="'.$video_url.'" class="viewVideos" title="live class" onclick=javascript:viewvideos(\''.$data['course_code'].'\'); ><i class="fa fa-music" alt="Video" title="Click here to view video(s)"></i></a>';
							}
							else
							{
								/*$video_link ='<i class="fa fa-video-camera" alt="No Video" title="No Videos"></i>';*/
